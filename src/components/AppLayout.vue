...
function updateDropHint() {
  ...
  if (target.type === 'sidebar') {
    dropHint.value = '释放后按文件夹名创建列表'
  } else if (target.type === 'playlist') {
    dropHint.value = '释放后加入此列表'
  } else {
    dropHint.value = '释放后加入当前列表'
  }
}

async function addPathsToPlaylist(paths: string[], playlistId: number): Promise<number> {
  try {
    const result = await invoke<{ added: number; skipped: number; failed: number }>(
      'library_add_files',
      { paths, playlistId }
    )
    await playlistStore.reloadPlaylistSongs(playlistId)
...
async function handleDroppedFiles(paths: string[]) {
  ...
  appStore.scanningMessage =
    totalAdded > 0 ? `已添加 ${totalAdded} 首歌曲` : '未发现音乐文件'
  setTimeout(() => {
    appStore.scanningMessage = ''
  }, 3000)
  appStore.saveConfig()
