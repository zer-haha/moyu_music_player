...
async function selectPlaylist(id: number) {
  await playlistStore.setCurrentPlaylist(id)
...
async function deletePlaylist(id: number) {
  ...
  if (!pl) return
  if (!confirm(`确定删除「${pl.name}」？\n歌曲文件不会被删除`)) return
  await playlistStore.removePlaylist(id)
  appStore.saveConfig()
}

async function clearPlaylist(id: number) {
  const pl = playlistStore.playlists.find((p) => p.id === id)
  if (!pl) return
  if (!confirm(`确定清空「${pl.name}」？\n歌曲文件不会被删除`)) return
  await playlistStore.clearPlaylist(id)
  appStore.saveConfig()
}
...
function ctxNewPlaylist() {
  hideContextMenu()
  startNewPlaylist()
}
function ctxRename() {
  if (contextMenu.value.playlist) startRename(contextMenu.value.playlist)
  hideContextMenu()
}
function ctxDelete() {
  if (contextMenu.value.playlist && contextMenu.value.playlist.kind !== 'default')
    deletePlaylist(contextMenu.value.playlist.id)
  hideContextMenu()
}
function ctxClear() {
  if (contextMenu.value.playlist) clearPlaylist(contextMenu.value.playlist.id)
  hideContextMenu()
}
function ctxAddFolder() {
  hideContextMenu()
  addFolderAsPlaylist()
}
...
async function onPlMouseUp() {
  ...
  if (ds.active && ds.startIndex >= 0 && ds.targetIndex >= 0 && ds.startIndex !== ds.targetIndex) {
    await playlistStore.movePlaylist(ds.startIndex, ds.targetIndex)
    appStore.saveConfig()
  }
    ...
    <div class="sidebar-title">
      ...
      <div class="title-actions">
        <button
          ...
          @click="addFolderAsPlaylist"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
          </svg>
        </button>
        <button class="add-btn" title="新建空列表" @click="startNewPlaylist">
    ...
    <div ref="listRef" class="playlist-list">
      <div v-if="showNewInput" class="playlist-item new-input">
        <input
          ...
          placeholder="输入列表名称"
          @keyup.enter="confirmNewPlaylist"
      ...
      <div
        v-for="(pl, idx) in playlistStore.playlists"
        :key="pl.id"
        class="playlist-item"
        ...
        :class="{
          ...
          'drag-target':
            dragState.active && dragState.targetIndex === idx && dragState.startIndex !== idx,
          draggable: pl.kind !== 'default',
        }"
        @mousedown="onPlMouseDown($event, idx)"
        @click="selectPlaylist(pl.id)"
        @contextmenu="onPlaylistContextMenu($event, pl)"
      >
        <div class="pl-icon">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <input
          v-if="isRenaming && renameId === pl.id"
          v-model="renameValue"
          class="rename-input"
        ...
        <template v-else>
          ...
          <span class="pl-count">{{ pl.tracks.length || pl.track_count || 0 }}</span>
          <svg
            v-if="pl.kind !== 'default'"
            class="drag-handle"
            width="10"
            height="14"
            viewBox="0 0 10 14"
